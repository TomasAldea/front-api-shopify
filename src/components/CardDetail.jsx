...
export const CardDetail = () => {
  ...
  const printDetail = (products) => {
    ...
    if (cardInfo) {
      ...
      let carouselArr = cardInfo.images.map((obj) => ({
        image: obj.src,
        caption: caption,
      }));

      if (cardInfo.images.length < 1) {
        const currentDomain = window.location.hostname;
        carouselArr = {
          image: `https://${currentDomain}/No_Image_Available.jpg`,
          caption: "",
        };
      }
    ...
    >
      ...
      {cardDetail && cardDetail.variants.length > 1 ? (
        <div className="info-container">
          ...
          <ul>
            {cardDetail.variants.map((product, index) => (
              <li onClick={() => selectItem(index, product.price)} key={index}>
        ...
        <div className="info-container">
          ...
          <div
            className={`price ${
              cardDetail?.variants.length > 1 ? "" : "extra-margin"
            }`}
          >
            Total: <span> {total}</span> €
          </div>
